...
abstract class Minion_Task {
	...
	public static function convert_task_to_class_name($task)
	{
		...
		return 'CLI_'.implode('_', array_map('ucfirst', explode(Minion_Task::$task_separator, $task)));
	}
	...
	{
		if (($task = self::get($options, 'task')) !== NULL)
		{
			unset($options['task']);
		}
		else if (($task = self::get($options, 0)) !== NULL)
		{
	...
	public function execute()
	{
		...
		if ( $this->_method != '_help' AND FALSE)
		{
